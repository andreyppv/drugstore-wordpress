...
include_once(__DIR__ . '/LimeLight.php');
...
class Membership extends LimeLight {
    ...
    function __construct($api_username, $api_password, $lime_light_url, $log_directory = '', $log_file = '', $log_delimiter = '|', $output_type = 'array'){
    ...
    //added more
    private function ShippingMethodView($shipping_id) {
        $this->method = 'shipping_method_view';
        $this->response = $this->APIConnect('shipping_id', $shipping_id);    
    }
    
    private function ShippingMethodFind($campaign_id, $search_type = '', $return_type = '', $criteria = '') {
        $fields = NULL;
        $values = NULL;
    ...
    public function GetResponse($do_what, $parameters){
        ...
        switch(strtolower($do_what)){
            ...
            //added more
            case 'shippingmethodview':
                $this->ShippingMethodView($parameters);
                break;
            case 'shippingmethodfind':
                $this->ShippingMethodFind($parameters[0], $parameters[1], $parameters[2], $parameters[3]);
                break;
        }   // END switch
