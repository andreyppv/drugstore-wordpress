	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		...
		<?php elseif ( 1 === count( $available_methods ) ) :  ?>
			<?php
                ...
                printf( '%3$s <input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d" value="%2$s" class="shipping_method"/>', $index, esc_attr( $method->id ), '' );
				do_action( 'woocommerce_after_shipping_rate', $method, $index );
                
                smash_display_item_shippings();
			?>
		<?php elseif ( ! WC()->customer->has_calculated_shipping() ) : ?>
			<?php //echo wpautop( __( 'Shipping costs will be calculated once you have provided your address.', 'woocommerce' ) ); ?>
            <?php smash_display_item_shippings(); ?>
		<?php else : ?>
