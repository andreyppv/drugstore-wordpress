...
class Subscriptio_Subscription
{
    ...
    public function populate()
    {
        ...
        $this->status = !empty($post_terms) ? $post_terms[0]->slug : '';
    ...
    public function create_from_order_item($order, $order_meta, $order_item_id, $order_item, $product, $product_meta, $renewal)
    {
        ...
        $this->update_subscription_details(array(
            ...
            // Properties needed for renewal orders
            'shipping_address'              => $renewal['shipping_address'],
            ...
            'renewal_all_items_meta'        => $order_item['item_meta'],
        ));
    ...
    public function create_from_all_order_items($order, $order_meta, $all_subs, $renewal)
    {
        ...
        $this->update_subscription_details(array(
            ...
            'renewal_order_tax'             => $renewal['renewal_order_tax'],
            'renewal_order_subtotal'        => $renewal['renewal_order_subtotal'],
            'renewal_order_total'           => $order_meta['_order_total'], //$renewal['renewal_order_total'],
            'renewal_order_currency'        => !empty($order_meta['_order_currency']) ? $order_meta['_order_currency'] : '',
            'renewal_prices_include_tax'    => !empty($order_meta['_prices_include_tax']) ? $order_meta['_prices_include_tax'] : '',
            'renewal_customer_ip_address'   => !empty($order_meta['_customer_ip_address']) ? $order_meta['_customer_ip_address'] : '',
            'renewal_customer_user_agent'   => !empty($order_meta['_customer_user_agent']) ? $order_meta['_customer_user_agent'] : '',
            ...
            // Save all order meta in case there's custom data added
            'renewal_all_order_meta'        => $order_meta['_order_total'],
            'renewal_all_items_meta'        => $order_item['item_meta'],
        ));
        ...
        foreach ($all_subs as $product_details) {
            $total_signup_fee += $product_details['product_post_meta']['_subscriptio_signup_fee'];
        ...
        $products_multiple = array();
         ...
         foreach ($all_subs as $product_details) {
            $products_multiple[] = array(
                ...
                'variation_id' => !empty($product_details['order_item']['variation_id']) ? $product_details['order_item']['variation_id'] : null,
    ...
    public function check_compatibility($all_subs)
    {
        ...
        foreach ($all_subs as $subs_product) {
            ...
            $product_meta_prev = $subs_product['product_post_meta'];
        }
    ...
    public function pay_by_order($order_id, $transaction)
    {
        ...
        try {
            ...
            if ($old_status == 'pending') {
                ...
                $update_subscription_details['started_readable'] = Subscriptio::get_datetime($update_subscription_details['started']);
                ...
                if (!empty($this->max_length_time_unit) && !empty($this->max_length_time_value)) {
                    ...
                    $update_subscription_details['expires_readable'] = Subscriptio::get_datetime($update_subscription_details['expires']);

                    // Schedule expiration event
                    if (!Subscriptio_Scheduler::schedule_expiration($this->id, $update_subscription_details['expires'])) {
            ...
            $update_subscription_details['payment_due_readable'] = Subscriptio::get_datetime($update_subscription_details['payment_due']);

            // Schedule next payment event
            if (!Subscriptio_Scheduler::schedule_payment($this->id, $update_subscription_details['payment_due'])) {
                throw new Exception(__('Failed scheduling next payment event.', 'subscriptio'));
            }

            // Schedule renewal order event
            if (!Subscriptio_Scheduler::schedule_order($this->id, self::calculate_renewal_order_time($update_subscription_details['payment_due']))) {
    ...
    public function pause()
    {
        ...
        if (Subscriptio::option('max_pause_duration') > 0) {
            ...
            $resumes_readable = Subscriptio::get_datetime($resumes);
            ...
            if (!Subscriptio_Scheduler::schedule_resume($this->id, $resumes)) {
                return;
            }
        }
        ...
        $this->update_subscription_details(array(
            ...
            'pre_paused_events' => Subscriptio_Scheduler::get_scheduled_events_timestamps($this->id),
            'paused_since'      => time(),
        ));

        // Unschedule almost all events (they were just saved to subscription property pre_paused_events)
        Subscriptio_Scheduler::unschedule_multiple(array(
            'subscriptio_scheduled_payment',
            'subscriptio_scheduled_order',
            'subscriptio_scheduled_reminder',
            'subscriptio_scheduled_suspension',
            'subscriptio_scheduled_cancellation',
            'subscriptio_scheduled_expiration',
        ), $this->id);
    ...
    public function resume()
    {
        ...
        Subscriptio_Scheduler::unschedule('subscriptio_scheduled_resume', $this->id);
    ...
    public function overdue()
    {
        ...
        // Send notifications
        Subscriptio_Mailer::send('payment_overdue', $this);
    }
    ...
    public function calculate_renewal_order_time($payment_due)
    {
        ...
        $fifteen_minutes = Subscriptio::$debug ? 0 : 900;
        $one_minute = Subscriptio::$debug ? 0 : 60;
    ...
    public function calculate_next_payment_time()
    {
        ...
        $twenty_minutes = Subscriptio::$debug ? 0 : 1200;
    ...
    public function calculate_expiration_time()
    {
        ...
        // Use scale of 1:10080 (1 minute = 1 week) when debugging
        $thirty_minutes = Subscriptio::$debug ? 0 : 1800;
    ...
    public function get_items()
    {
        ...
        // Currently we can only have one product per subscription (and most probably it will stay this way)
        if (!empty($this->product_id)) {

            $item = array(
                'product_id'    => $this->product_id,
                'quantity'      => $this->quantity,
                'total'         => $this->renewal_line_total,
                'tax'           => $this->renewal_line_tax,
                'deleted'       => false,
            );

            // Is this a variable product?
            $item['name'] = !empty($this->variation_id) ? sprintf(__('Variation #%1$s of', 'subscriptio'), $this->variation_id) . ' ' : '';

            // Get quantity
            $quantity = !empty($this->quantity) ? $this->quantity : 1;

            // Is this product still active?
            if (Subscriptio::product_is_active($this->product_id)) {

                // Get current product name
                $item['name'] .= get_the_title($this->product_id);

                // Is variation still active?
                if ($this->variation_id && !Subscriptio::product_is_active($this->variation_id)) {
                    $item['name'] .= ' (' . __('variation deleted', 'subscriptio') . ')';
                }
            }
            else {
                $item['name'] .= $this->product_name;
    ...
    {
        return Subscriptio::get_formatted_price($price, $this->renewal_order_currency);
    }
    ...
    public function check_pause_limit()
    {
        ...
        if (!empty($user_current_pause_amount)) {
            $user_current_pause_amount = Subscriptio::unwrap_post_meta($user_current_pause_amount);
