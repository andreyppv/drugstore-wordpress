...
class Subscriptio_Order_Handler
{
    ...
    public function new_order($order_id)
    {
        ...
        $multiproduct_subscription = Subscriptio::multiproduct_mode();
        ...
        if ($multiproduct_subscription) {
            ...
            $cart_item_key_first = array_shift(array_keys($woocommerce->cart->cart_contents));
        }
        ...
        foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item) {
            ...
            if (Subscriptio_Subscription_Product::is_subscription($id)) {
                ...
                // Add taxes always for regular subscription and only on last item for multiproduct
                if (!$multiproduct_subscription || $multiproduct_subscription && $itemcount == count($woocommerce->cart->cart_contents)) {
    ...
    public function new_order_placed($order_id, $posted)
    {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            try {
                ...
                if ($multiproduct_subscription && $subscription->check_compatibility($all_subs)) {
    ...
    public static function create_renewal_order($subscription)
    {
        ...
        $other_meta_fields = array(
            ...
            '_order_subtotal'       => $subscription->renewal_order_subtotal,
            // Use subtotal instead of total if shipping charge is off for renewal orders
            '_order_total'          => $charge_shipping ? $subscription->renewal_order_total : $subscription->renewal_order_subtotal,
        ...
        foreach ($subscription->renewal_all_order_meta as $meta_key => $meta_value) {
            if (!isset($billing_shipping_fields[$meta_key]) && !isset($other_meta_fields[$meta_key])) {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            if (!empty($all_subs)) {
                $product_id_to_use = !empty($all_subs[$i]['variation_id']) ? $all_subs[$i]['variation_id'] : $all_subs[$i]['product_id'];
            ...
            // Get totals
            $renewal_line_subtotal = $subscription->renewal_line_subtotal;
            $renewal_line_subtotal_tax = $subscription->renewal_line_subtotal_tax;
            $renewal_line_total = $subscription->renewal_line_total;
            $renewal_line_tax = $subscription->renewal_line_tax;
            ...
            foreach ($subscription->renewal_all_items_meta as $item_meta_key => $item_meta_value) {
        ...
        $payment_due_timestamp = Subscriptio_Scheduler::get_scheduled_event_timestamp('subscriptio_scheduled_payment', $subscription->id);
