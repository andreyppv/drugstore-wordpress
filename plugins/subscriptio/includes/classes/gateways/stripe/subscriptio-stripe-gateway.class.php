...
class Subscriptio_Stripe_Gateway extends WC_Payment_Gateway
{
    ...
    public function process_payment($order_id)
    {
        ...
        if (empty($selected_card)) {
            Subscriptio::add_woocommerce_frontend_error(__('Card is not selected.', 'subscriptio-stripe') . ' ' . __('We have not charged your card. Please try again.', 'subscriptio-stripe'));
            return;
        }
        ...
        if (!$order) {
            Subscriptio::add_woocommerce_frontend_error(__('Order not found.', 'subscriptio-stripe') . ' ' . __('We have not charged your card. Please try again.', 'subscriptio-stripe'));
            return;
        }
        ...
        if (!is_user_logged_in()) {
            ...
            if (empty($token)) {
                Subscriptio::add_woocommerce_frontend_error(__('Authorization token not set.', 'subscriptio-stripe') . ' ' . __('We have not charged your card. Please try again.', 'subscriptio-stripe'));
                return;
            }
        ...
        // Otherwise charge by sending a customer id / card id pair
            ...
            if (!is_array($user_card_details)) {
                Subscriptio::add_woocommerce_frontend_error($user_card_details . ' ' . __('We have not charged your card. Please try again.', 'subscriptio-stripe'));
                return;
            }
        ...
        if (is_string($response)) {
            Subscriptio::add_woocommerce_frontend_error($response);
            return;
        }
    ...
    public function get_card($order, $user_id, $token, $selected_card)
    {
        ...
        if (empty($customer_id)) {
            ...
            // Errors?
            if (!is_object($response) || !isset($response->id) || !isset($response->default_source)) {
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'capture' => array(
                ...
                'description'   => __('If unchecked, an authorization is issued at the time of purchase but the charge itself is captured when you start processed the order. Uncaptured charges expire in 7 days.', 'subscriptio-stripe'),
                'default'       => 'yes',
            ),
            'title' => array(
